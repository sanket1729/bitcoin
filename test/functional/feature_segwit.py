...
class SegWitTest(BitcoinTestFramework):
    ...
    def success_mine(self, node, txid, sign, redeem_script=""):
        ...
        block = node.generatetoaddress(1, node.get_deterministic_priv_key().address)
    ...
    def run_test(self):
        self.nodes[0].generatetoaddress(161, self.nodes[0].get_deterministic_priv_key().address) #block 161
        ...
        assert(tmpl['transactions'][0]['hash'] == txid)
        assert(tmpl['transactions'][0]['sigops'] == 2)
        self.nodes[0].generatetoaddress(1, self.nodes[0].get_deterministic_priv_key().address) #block 162
        ...
        self.nodes[0].generatetoaddress(1, self.nodes[0].get_deterministic_priv_key().address) #block 163
        sync_blocks(self.nodes)
        ...
        self.nodes[0].generatetoaddress(260, self.nodes[0].get_deterministic_priv_key().address) #block 423
        sync_blocks(self.nodes)
        ...
        self.nodes[2].generatetoaddress(4, self.nodes[2].get_deterministic_priv_key().address) # blocks 428-431

        self.log.info("Verify previous witness txs skipped for mining can now be mined")
        assert_equal(len(self.nodes[2].getrawmempool()), 4)
        block = self.nodes[2].generatetoaddress(1, self.nodes[2].get_deterministic_priv_key().address) #block 432 (first block with new rules; 432 = 144 * 3)
        sync_blocks(self.nodes)
        ...
        self.nodes[0].generatetoaddress(1, self.nodes[0].get_deterministic_priv_key().address) # Mine a block to clear the gbt cache
